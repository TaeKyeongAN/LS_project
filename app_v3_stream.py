...
main_tab, load_tab, time_tab, alert_tab, bill_tab, report_tab = st.tabs(
    ["메인 대시보드", "부하/그룹 분석", "피처 분석", "피크 & 알람/시뮬레이션", "한전 고지서/요금", "리포트"]
)
...
# Tabs 구성
# =========================================
feature_tab, time_tab = st.tabs(["기본 피처 분석", "패턴 분석"])

# =========================================
# Feature Overview (정규화 버전)
# =========================================
...
with feature_tab:
    ...
    # --------------------------
    if selected_feats:
        cols_to_scale = ["전기요금(원)"] + selected_feats
    else:
        cols_to_scale = ["전기요금(원)"]
...
with time_tab:
    ...
    with tab2:
        ...
        # ✅ x축 binning 방지: 시간 카테고리화 + nbinsx=24 설정
